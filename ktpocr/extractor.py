...
class KTPOCR(object):
    def __init__(self, image, image2):
        ...
        self.th, self.threshed = cv2.threshold(self.gray, 150, 255, cv2.THRESH_TRUNC)
    ...
    def Similar(self,img1,img2):
        ...
        distance = face_recognition.face_distance([img_encoding], img_encoding2)
        ...
        for word in extracted_result.split("\n"):
            ...
            if "Lahir" in word:
                ...
                try:
                    ...
                    self.result.tanggal_lahir = re.search("[0-9]+", word[-1])[0]
                self.result.tempat_lahir = word[-1].replace(self.result.tanggal_lahir, '').replace(",", '').replace(":", "").replace(".","").strip()
            ...
            if 'Alamat' in word:
                self.result.alamat = self.word_to_number_converter(word).replace("Alamat ","").replace(".","").replace(":","").replace("!","I")
            ...
            if "Desa" in word:
                ...
                self.result.kelurahan_atau_desa = self.result.kelurahan_atau_desa.replace(":","").replace("-","").strip()
